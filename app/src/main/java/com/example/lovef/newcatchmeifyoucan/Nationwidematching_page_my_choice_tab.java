 ...
 */
public class Nationwidematching_page_my_choice_tab extends TabActivity{
    Context mContext;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.nationwidematching_page_my_choice_tab);

        tabhost = getTabHost();
        //새로운 선택에 보내는 tab 시작
        tab = new Intent().setClass(Nationwidematching_page_my_choice_tab.this,Nationwidematching_page_my_choice.class);
        ...
        choice_page_tap = tabhost.newTabSpec("Nationwide_choice").setIndicator("새로운 선택").setContent(tab);
        tabhost.addTab(choice_page_tap);
        //새로운 선택에 보내는 tab 끝

        //지난 선택에 보내는 메소드 시작(아마 여기서 직접 변수를 넘겨주는 일은 없을듯, 여기서는 단순히 tab만 추가)
        tab = new Intent().setClass(Nationwidematching_page_my_choice_tab.this,Nationwidematching_page_my_choice_before.class);
        choice_page_tap = tabhost.newTabSpec("Nationwide_choice").setIndicator("지난 선택").setContent(tab);
        tabhost.addTab(choice_page_tap);
        //지난 선택에 보내는 메소드 끝
